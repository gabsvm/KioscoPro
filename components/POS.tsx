    ...
    <>
      <div className="flex flex-col md:flex-row h-[calc(100vh-180px)] md:h-[calc(100vh-140px)] gap-4 md:gap-6 relative">
        <div className={`flex-1 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden ${mobileView === 'CART' ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-2 md:p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-3 bg-slate-50 pb-24 md:pb-4 custom-scrollbar relative" style={{ alignItems: 'start', gridAutoRows: '1fr' }}>
              ...
              if ('parts' in item) {
                ...
                return (
                  ...
                  >
                    ...
                    {hasPromo && <div className="absolute top-1.5 left-1/2 -translate-x-1/2 bg-indigo-500 text-white text-[8px] font-bold px-1.5 py-px rounded-full flex items-center gap-0.5"><Tag size={8} /> PROMO</div>}
                    ...
                    {/* Stock & Price - always at bottom */}
                    <div className="flex flex-wrap justify-between items-center gap-1 mt-2 pt-1.5 border-t border-slate-100">
                      <span className={`text-[9px] md:text-[10px] px-1.5 py-px rounded font-medium ${product.stock > 10 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>Stock: {product.stock}</span>
                      <span className="font-bold text-xs md:text-sm text-brand-600">{product.isVariablePrice ? '$-.--' : formatCurrency(product.sellingPrice)}</span>
                    </div>
                  </div>
                );
              }
