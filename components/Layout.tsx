...
const Layout: React.FC<LayoutProps> = ({ currentView, setView, children, userEmail, isGuest, onLogout, userRole, onToggleRole, storeProfile }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      <aside className={`hidden md:flex w-64 ${userRole === 'ADMIN' ? 'bg-slate-900/95' : 'bg-slate-800/95'} backdrop-blur-2xl text-white flex-col shadow-2xl z-20 transition-all duration-300 border-r border-white/5`}>
        <div className="p-6 flex items-center gap-3 border-b border-white/10 relative overflow-hidden">
          {/* Subtle glow effect behind logo */}
          <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-brand-500/20 to-transparent pointer-events-none"></div>
          {appLogo ? (
        ...
        <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto custom-scrollbar">
          {visibleNavItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group relative overflow-hidden ${currentView === item.id
                  ? (userRole === 'ADMIN' ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/30' : 'bg-orange-500 text-white shadow-lg shadow-orange-500/30')
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative w-full bg-slate-50/50">
        <header className="h-16 bg-white/70 backdrop-blur-md border-b border-white/40 flex items-center justify-between px-4 md:px-8 shadow-sm z-30 shrink-0 sticky top-0">
              ...
              {isGuest ? (
                ...
                <div className="flex items-center gap-3">
                  <div className="hidden sm:flex flex-col items-end">
                    ...
                    <span className={`text-[9px] font-black tracking-widest uppercase ${userRole === 'ADMIN' ? 'text-brand-500' : 'text-orange-500'}`}>{userRole === 'ADMIN' ? 'ADMIN' : 'SELLER'}</span>
                  </div>
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white font-bold shadow-md transition-transform hover:scale-105 cursor-pointer ${userRole === 'ADMIN' ? 'bg-gradient-to-br from-brand-400 to-brand-600 border-2 border-white' : 'bg-gradient-to-br from-orange-400 to-orange-600 border-2 border-white'}`}>
                  ...
                  <div className="p-2 space-y-1">
                    ...
                    {!isGuest && (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors text-sm font-bold ${userRole === 'ADMIN'
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-lg border-t border-slate-200/50 flex justify-around items-center h-16 z-20 shadow-[0_-10px_25px_-5px_rgba(0,0,0,0.05)] overflow-x-auto no-scrollbar">
          {visibleNavItems.map(item => (
            <button
              key={item.id}
              onClick={() => setView(item.id)}
              className={`flex flex-col items-center justify-center min-w-[70px] h-full space-y-1 active:bg-slate-50 ${currentView === item.id ? (userRole === 'ADMIN' ? 'text-brand-600' : 'text-orange-600') : 'text-slate-400'
                }`}
            >
